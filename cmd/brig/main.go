...
func dispatch(args []string) error {
	...
	case "shell":
		// Removed ahead of 0.3, the first retired spelling to go. shell was
		// never translated to sh: it shared sh's branch below, so every change
		// to sh changed it too, without the checks sh makes on the way in.
		// Refused before anything is looked up, so the answer is the same
		// whatever follows the verb, and without the deprecation notice, since
		// nothing here works any more to be deprecated.
		return usagef("`brig shell` was removed. `brig sh <ref>` opens a login shell, " +
			"and `brig sh <ref> <command>` runs one")
	case "exec":
		// exec ran a command, and sh runs one too, because sh is both of the
		// verbs it replaced: which one you want is said by whether you typed a
		// command, not by which word you reached for.
		//
		// exec keeps its own branch below rather than being translated to sh.
		// It runs its argv directly where sh runs it through `bash -lc`, so a
...
// run forwards the agent's argv; sh -- with exec, the retired spelling it
// replaces -- turns the tail into the guest command. Every other verb takes a
// ref and nothing after it.
//
