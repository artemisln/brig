...
| `BRIG_KEYCHAIN` (global only) | (unset) | macOS only: `native` reaches the login keychain through Security.framework, `security` through `security(1)`. Unset, a signed build takes `native` and an ad-hoc signed one `security`. See [secrets.md](secrets.md#two-ways-into-the-keychain) |
