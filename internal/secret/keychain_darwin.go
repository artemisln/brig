...
// maxLine is security -i's line buffer. Measured on macOS 15: 4095 characters
...
func (k keychain) writeArgs(name string, update bool, p Provenance) ([]string, error) {
	...
	case update:
		// -U keeps attributes not named on the command, so an update without
		// -j would keep the previous value's provenance. An empty -j clears it.
		args = append(args, "-j", "")
	}
	if update {
		args = append(args, "-U")
	}
	// getopt stops at the first non-option, so -w must be last and no
	// keychain can be named.
	return append(args, "-w"), nil
}

// interactiveLine joins args and the encoded value into one security -i
// command. Only the label, the kind and an empty -j need quotes: the name is
// restricted by ValidName, and the value and provenance are base64.
func interactiveLine(args []string, encoded string) string {
	...
	return strings.Join(append(quoted, encoded), " ")
}

// Write stores a value with its provenance, creating or updating. A zero
// Provenance omits -j on create and clears it on update.
//
// The value is base64-encoded so newlines, NUL and non-UTF-8 bytes survive
// security's line-based input.
//
// A command that fits maxLine goes to security -i on stdin, keeping the value
// out of argv. A longer one is passed as arguments, because security -i
// truncates it and the prompt form (-w last) truncates at 128 bytes. On that
// path the encoded value is visible in ps and to exec auditing while security
// runs.
func (k keychain) Write(name string, value []byte, p Provenance, update bool) error {
	args, err := k.writeArgs(name, update, p)
	if err != nil {
		return err
	}
...
// write is Create and Update's path, with no provenance.
func (k keychain) write(name string, value []byte, update bool) error {
	return k.Write(name, value, Provenance{}, update)
}

// verify reads back what Write stored, since security truncates an over-long
// line without an error. A mismatched create is deleted; a mismatched update
// cannot be undone and is only reported.
